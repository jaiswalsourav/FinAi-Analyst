...
export default function StockDetail({ symbol }) {
	...
	return (
		<div className="result-box">
			<div className="panel-heading"><div><span className="eyebrow">Market pulse</span><h3>Stock Details</h3></div><span className="symbol-pill">{symbol}</span></div>
			{loading && <div>Loading stock data...</div>}
			{error && <div style={{ color: '#ef4444' }}>{error}</div>}
			{info && <div>
				<div className="quote-grid"><div className="quote-card quote-card-primary"><span>Last price</span><strong>${price}</strong></div><div className="quote-card"><span>Change</span><strong>{change}</strong></div><div className="quote-card"><span>Today</span><strong>{percent}</strong></div></div>
				{closeData.length > 1 && <div className="mini-chart"><div className="section-heading"><span>Recent close trend</span><small>Last {closeData.length} sessions</small></div><ResponsiveContainer width="100%" height={190}><LineChart data={closeData} margin={{ top: 8, right: 8, left: -22, bottom: 0 }}><CartesianGrid stroke="#25344a" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" stroke="#91a4bf" tick={{ fontSize: 10 }} /><YAxis stroke="#91a4bf" tick={{ fontSize: 10 }} domain={['dataMin', 'dataMax']} /><Tooltip contentStyle={{ background: '#132238', border: '1px solid #2c405c', borderRadius: 8 }} formatter={(value) => [`$${Number(value).toFixed(2)}`, 'Close']} /><Line type="monotone" dataKey="close" stroke="#63d7bd" strokeWidth={2.5} dot={false} /></LineChart></ResponsiveContainer></div>}
				{closeData.length > 0 && <div className="recent-table-wrap"><table className="recent-table"><thead><tr><th>Date</th><th>Close</th></tr></thead><tbody>{closeData.slice(-5).reverse().map((point) => <tr key={point.date}><td>{point.date}</td><td>${point.close.toFixed(2)}</td></tr>)}</tbody></table></div>}
			</div>}
		</div>
	);
}
